...
### ADR-023: `SRS_SDS_Report_CrossBorderTax.docx` (formal submission report) updated to reflect the dual-Act scope
- **Date:** 2026-09-20
- **Decision:** Closed the remaining gap flagged in ADR-022 ("the formal submission doc... was NOT touched"). Added a dated scope note under §1.1 (matching the note already added to the two Markdown docs), and corrected every specific claim that assumed a single Act: §1.1 Purpose, both relevant §1.2 Scope bullets, FR-3 in the requirements table, §3.2 Module Design, §3.4 UI Design, and §3.6 Sequence Diagram.
- **Also added:** FR-13 in this report's own requirements table, mirroring `System_Design_and_Requirements.md`'s FR-16 (a partnership firm's presumptive income is never further reduced by partner remuneration) -- this report's FR table doesn't share numbering with the living docs (it only ever listed the 12 MVP-scoped FRs, not the stretch/out-of-scope ones), so it's FR-13 here rather than FR-16, same requirement.
- **Not added:** Persona F's general-business routing was folded into FR-3's rewritten text, same treatment as ADR-022, rather than a new FR row -- it's a mode of the same computation.
- **Mechanics:** Edited directly in `word/document.xml` (unzip → edit → rezip), since this is a formal Word document, not Markdown. Validated against the document's XSD and rendered to PDF page images to visually confirm every change before delivering -- all nine edits (nine text passages plus the new FR-13 table row, matching the alternating row-shading pattern) confirmed rendering correctly with no layout drift; the Table of Contents page numbers are unaffected (still 13 pages total, same as the original).
- **Source:** `docs/SRS_SDS_Report_CrossBorderTax.docx`; ADR-015, ADR-016, ADR-022.
