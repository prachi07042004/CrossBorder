...
Note on verification rigor (2026-09-20): two gaps found in the 2026-09-20 review were closed the same day -- Rule 128's "direct visual page-read" claim was ambiguous (source PDF over the page-image tool's 100MB limit, no explicit qpdf-split note like the other affected files) and has now been genuinely re-confirmed; and Persona F's core legal claim (Section 44AD(6)'s exclusion list) had never actually been curated, only recalled -- now added and confirmed verbatim. Still open from that same review: whether the four Income-tax Act, 2025 files (Section 58, 35(e), 62, 159) have the same 100MB-source-PDF ambiguity as Rule 128 did -- blocked on re-obtaining the source PDF (`Income_Tax_Act_2025_as_amended_by_FA_Act_2026.pdf`), which is no longer available to re-check against.
