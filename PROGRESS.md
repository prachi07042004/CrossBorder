...
- **Not closed:** the same verification-method ambiguity likely affects the four Income-tax Act, 2025 files (Section 58, 35(e), 62, 159) -- all dated the same batch as the files with the confirmed overstatement, source PDF size never recorded. Blocked on re-obtaining `Income_Tax_Act_2025_as_amended_by_FA_Act_2026.pdf`, which isn't available in the cloud workspace or the project folder right now.
- **Next:** re-supply the 2025 Act PDF and re-check those four files the same way; then commit the full batch; then a Section-58/FY2026-27 persona; then Phase 1 proper.
- **Blocked on:** the 2025 Act source PDF, for the four-file re-check only -- nothing else blocks the commit.
