...
- [~] DTAA Article 25 (foreign tax credit) logic -- `compute_article_25_relief()` models the `min(US tax paid, India tax attributable)` mechanism, but both cap inputs are deliberately left `None`/pending rather than computed: US tax paid needs 26 U.S.C. Section 871(b)/872 + Form 1040-NR research not yet done (ADR-012), India tax attributable needs a progressive slab-rate calculator not yet built. Genuinely blocked, not decided against -- see `docs/personas.md` Persona C and this session's corpus-sufficiency discussion.
...
- [~] Unit test suite passing against all defined scenarios, including boundary cases -- 33/33 passing: the non-treaty personas (A, B1, B2, G), Act-selection boundary, threshold/proviso boundary cases (6), eligibility-gate cases (4), Personas C/D (DTAA) plus the 90-day boundary, fixed-base-with-zero-attribution edge case, and the both-conditions-met precedence case, and now Persona E (firm remuneration not deducted, 2 new). Persona F (44AD misrouting) still not covered -- tracked as a follow-up increment
