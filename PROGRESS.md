...
- [~] Unit test suite passing against all defined scenarios, including boundary cases -- 31/31 passing: the non-treaty personas (A, B1, B2, G) plus Act-selection boundary, threshold/proviso boundary cases (6), eligibility-gate cases (4), and now Personas C/D (DTAA) plus the 90-day boundary, fixed-base-with-zero-attribution edge case, and the both-conditions-met precedence case (7 new). Personas E/F (firm remuneration, misrouting) still not covered -- tracked as follow-up increments
